...
| `codexConfig` | Codex 专用配置路径 | 可选，指向独立的 Codex 配置文件                        |

### Codex 配置选择

Codex CLI 默认读取用户级配置（`~/.codex/config.yaml` 或环境变量）。在某些场景下，技能调用的 Codex 可能需要与用户日常使用的配置不同（如使用不同的模型、不同的 API 端点、不同的审批策略）。

- **不指定 `codexConfig`**：使用用户默认的 Codex 配置。适合大多数情况。
- **指定 `codexConfig`**：指向一个独立的配置文件路径。调用 Codex 时通过环境变量或 CLI 参数覆盖默认配置。适合需要差异化行为的场景。
...
使用 Git worktree 隔离 Codex 的工作目录，防止并发修改冲突。阅读 [Worktree 隔离指南](./references/worktree.md)。
